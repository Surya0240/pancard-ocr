...
(newW, newH) = (args["width"], args["length"])
rW = W / float(newW)
rH = H / float(newH)
...
for (startX, startY, endX, endY) in sorted(boxes, key=lambda x: x[1]):
	...
	if endY-startY > endX-startX:
		continue
	sampled = orig[startY:endY, startX:endX]
	# text = perform_ocr(sampled)
	# if "signature" in text.lower().strip('.').strip() :
	# 	break
	found_group = False
	for group in groups:
...
for group in groups:
	sampled = orig[group['startY']:group['endY'], group['startX']:group['endX']]
